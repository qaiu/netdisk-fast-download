...
DEFAULT_JVM_OPTS="-Xmx${JVM_XMX:-512M} -Xss${JVM_XSS:-512k}"
if [ -n "${JVM_MAX_DIRECT_MEMORY:-}" ]; then
  DEFAULT_JVM_OPTS="$DEFAULT_JVM_OPTS -XX:MaxDirectMemorySize=$JVM_MAX_DIRECT_MEMORY"
fi
